...
export function MaintenanceMassPlannerV2({
    ...
    setDrafts((drafts) =>
      drafts.map((draft) =>
        draft.building.id === buildingId
          ? {
              ...draft,
              externalTechnicians: externalTechnicians.filter((t) => technicianIds.includes(t.id)),
            }
          : draft
      )
    );
  ...
  const handleExternalNamesChange = (buildingId: string, names: string[]) => {
    setDrafts((drafts) =>
      drafts.map((draft) =>
        draft.building.id === buildingId ? { ...draft, externalNames: names } : draft
      )
    );
  };
  ...
  const handleDurationChange = (buildingId: string, duration: number) => {
    setDrafts((drafts) =>
      drafts.map((draft) =>
        draft.building.id === buildingId ? { ...draft, duration } : draft
      )
    );
  };
  ...
  const handleFixedChange = (buildingId: string, isFixed: boolean) => {
    setDrafts((drafts) =>
      drafts.map((draft) =>
        draft.building.id === buildingId ? { ...draft, is_fixed: isFixed } : draft
      )
    );
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (assignments.length === 0) {
        setError(
          'No hay asignaciones para guardar. Selecciona al menos un técnico o nombre externo por edificio.'
        );
        setLoading(false);
        return;
      }
        ...
        <div className="w-64 flex-shrink-0">
          ...
          <div className="mt-6">
            ...
            <div className="text-xs text-gray-500 mt-1">
              Técnicos externos agregados estarán disponibles en la tabla.
            </div>
          </div>
        </div>
              ...
              <table className="min-w-full border text-base mb-4">
                ...
                <tbody>
                  {drafts.length === 0 ? (
                    ...
                    drafts.map((draft) => (
                      <tr
                        key={draft.building.id}
                        className={draft.status !== 'ok' ? 'bg-red-50' : ''}
                      >
                        <td className="border px-4 py-2 font-semibold text-lg">
                          {draft.building.name}
                        </td>
