              ...
              {filtered.map((client) => (
                <tr key={client.id} className="border-b border-slate-50 hover:bg-slate-50/70">
                  <td className="px-4 py-2 align-top">
                    ...
                    {client.building_name && <div className="text-xs text-slate-500">Edificio: {client.building_name}</div>}
                    <div className="text-[10px] text-slate-400">Creado: {new Date(client.created_at).toLocaleDateString()}</div>
                  </td>
                  <td className="px-4 py-2 align-top">
                  ...
                  <td className="px-4 py-2 align-top text-center">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold ${client.is_active ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-slate-50 text-slate-500 border border-slate-100'}`}>
                      {client.is_active ? 'Activo' : 'Inactivo'}
                    </span>
                  </td>
                  <td className="px-4 py-2 align-top">
                    <div className="flex items-center justify-end gap-2">
                      <button type="button" onClick={() => openEdit(client)} className="p-1.5 rounded-md hover:bg-blue-50 text-blue-600" title="Editar cliente">
