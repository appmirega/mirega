...
import { Shield, Users, Save, Eye, EyeOff, CheckCircle, XCircle } from 'lucide-react';
import { ALL_VIEWS, getViewsForRole } from '../../utils/viewPermissions';
...
export function DeveloperPermissionsPanel() {
  ...
  const loadViewPermissions = async (profileId: string) => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        const userRole = profileData.role as 'admin' | 'technician' | 'client';
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-slate-900 flex items-center gap-3">
          ...
          Control de Vistas por Perfil
        </h1>
        <p className="text-slate-600 mt-1">
          Activa o desactiva las vistas disponibles para cada perfil de usuario
        </p>
      </div>
